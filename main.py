...
class Bonus(GameSprite):
    ...
    def catch(self, game_sprite):
        if sprite.collide_rect(self, game_sprite):
            self.do_effect(game_sprite)
            self.kill()
...
class IncreaseDamageBonus(Bonus):
    ...
    def do_effect(self, game_sprite):
        game_sprite.dmg += 1
        print(game_sprite, "dmg =" , game_sprite.dmg)
...
def update_score(amount):
    ...
    score += amount
    text_score = font_interface.render("Рхунок: " + str(score), True, (255, 255, 255))
